...
class InfUFluxPipeline:
    def __init__(
        ...
        try:
            ...
            print("No InfiniteYou model found. Downloading from HuggingFace `ByteDance/InfiniteYou` to `/group-volume/Aman-Contents/logs/models2/InfiniteYou` ...")
            snapshot_download(repo_id='ByteDance/InfiniteYou', local_dir='/group-volume/Aman-Contents/logs/models2/InfiniteYou', local_dir_use_symlinks=False)
            infu_model_path = os.path.join('/group-volume/Aman-Contents/logs/models2/InfiniteYou', f'infu_flux_{infu_flux_version}', model_version)
            infusenet_path = os.path.join(infu_model_path, 'InfuseNetModel')
            self.infusenet = FluxControlNetModel.from_pretrained(infusenet_path, torch_dtype=torch.bfloat16)
            insightface_root_path = '/group-volume/Aman-Contents/logs/models2/InfiniteYou/supports/insightface'
        if quantize_8bit:
